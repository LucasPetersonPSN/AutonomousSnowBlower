...
const int motorRight1 = 12;  //TO IN3
const int motorRight2 = 13;  //TO IN4
...
void checkUltrasonic() {
    ...
    long distRear = readUltrasonic(trigRear, echoRear);

    if (distLeft <= 25){  //Flag is 1,0
        digitalWrite(0, HIGH);
        digitalWrite(1, LOW);
    }
    else if (distRight <= 25){  //Flag is 0,1
        digitalWrite(0, LOW);
        digitalWrite(1, HIGH);
    }
    else if (distRear <= 25){  //Flag is 1,1
        digitalWrite(0, HIGH);
        digitalWrite(1, HIGH);
    }
    else{
...
void loop() {
    ...
    *//*
    ...
    Serial.println(distLeft + "/" + distRight + "/" + distRear + "-");*/
    
    /*
    Serial.print(readUltrasonic(trigLeft, echoLeft));
    Serial.print("/" );  //Separator ASCII 47
    Serial.print(readUltrasonic(trigRight, echoRight));
    Serial.print("/");  //Separator ASCII 47
    Serial.print(readUltrasonic(trigRear, echoRear));
    Serial.println("-");  //End of transmission ASCII 45
    */
